...
def test_prepare_for_training_clones_interval_start_blocks():
    ...
    assert loss.double_interval_specs[0].end == 2
    assert loss.single_interval_specs[0].start == 0
    assert loss.single_interval_specs[0].end == 1

    with torch.no_grad():
        transformer.transformer_blocks[1].weight.fill_(3.0)

    assert not torch.equal(
        loss.student_double_blocks[0].weight,
        transformer.transformer_blocks[1].weight,
    )
