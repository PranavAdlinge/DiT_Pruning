...
class Flux2KleinPipeline(DiffusionPipeline, Flux2LoraLoaderMixin):
    ...
    @staticmethod
    def load_distill_config(pretrained_model_name_or_path: str | os.PathLike) -> dict[str, list[list[int]]] | None:
        config_path = os.path.join(pretrained_model_name_or_path, "distill_config.json")
        ...
        return {
            "double_layer_indexs": distill_config.get("double_layer_indexs") or [],
            "single_layer_indexs": distill_config.get("single_layer_indexs") or [],
        }

    def configure_distill(
        self,
        ...
        if self.transformer is None:
            raise ValueError("`configure_distill(...)` requires the pipeline to have a transformer.")

        self.transformer.init_distill(
            double_layer_indexs=double_layer_indexs,
            single_layer_indexs=single_layer_indexs,
        )
        self.transformer.enable_distill_inference(enabled)
        return self

    def load_distill_lora_weights(
        self,
        pretrained_model_name_or_path_or_dict: str | dict[str, torch.Tensor],
        distill_config: dict[str, list[list[int]]] | None = None,
        enabled: bool = True,
        adapter_name: str | None = None,
        **kwargs,
    ):
        if distill_config is None and isinstance(pretrained_model_name_or_path_or_dict, (str, os.PathLike)):
            distill_config = self.load_distill_config(pretrained_model_name_or_path_or_dict)

        if distill_config is None:
            raise ValueError(
                "Distillation LoRA loading requires a `distill_config` or a `distill_config.json` file next to the weights."
            )

        self.configure_distill(
