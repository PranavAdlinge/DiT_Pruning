...
def _validate_distill_intervals(
    layer_indexs: list[list[int]] | tuple[tuple[int, int], ...] | None,
    num_layers: int,
    stream_name: str,
) -> list[tuple[int, int]]:
    ...
    normalized_layer_indexs: list[tuple[int, int]] = []
    for raw_layer_index in layer_indexs:
        if len(raw_layer_index) != 2:
            raise ValueError(
                f"Each {stream_name} distillation interval must contain exactly two indices. Received {raw_layer_index}."
            )

        start, end = int(raw_layer_index[0]), int(raw_layer_index[1])
        if start < 0 or end < 0:
            raise ValueError(f"{stream_name} distillation indices must be non-negative. Received {(start, end)}.")
        if start > end:
            raise ValueError(
                f"{stream_name} distillation intervals must satisfy start <= end. Received {(start, end)}."
            )
        if end >= num_layers:
            raise ValueError(
                f"{stream_name} distillation interval {(start, end)} exceeds the available layer range "
                f"[0, {num_layers - 1}]."
            )

        normalized_layer_indexs.append((start, end))

    normalized_layer_indexs.sort(key=lambda x: x[0])
    previous_end = -1
    for start, end in normalized_layer_indexs:
        if start <= previous_end:
            raise ValueError(
                f"{stream_name} distillation intervals must be non-overlapping. Received {normalized_layer_indexs}."
            )
        previous_end = end

    return normalized_layer_indexs


def _l2_normalize_last_dim(hidden_states: torch.Tensor, eps: float = 1e-6) -> torch.Tensor:
    return hidden_states / hidden_states.norm(p=2, dim=-1, keepdim=True).clamp_min(eps)
...
class Flux2Transformer2DModel(
    ...
    def _has_distill_blocks(self) -> bool:
        return (
            hasattr(self, "double_layer_indexs")
            and hasattr(self, "single_layer_indexs")
            and (
                len(getattr(self, "double_layer_indexs", [])) > 0 or len(getattr(self, "single_layer_indexs", [])) > 0
            )
        )

    def enable_distill_inference(self, enabled: bool = True):
        self._use_distill_inference = bool(enabled)

    def init_distill(
        self,
        double_layer_indexs: list[list[int]] | tuple[tuple[int, int], ...] | None = None,
        single_layer_indexs: list[list[int]] | tuple[tuple[int, int], ...] | None = None,
    ):
        double_layer_indexs = _validate_distill_intervals(
            double_layer_indexs, len(self.transformer_blocks), "double-stream"
        )
        single_layer_indexs = _validate_distill_intervals(
            single_layer_indexs, len(self.single_transformer_blocks), "single-stream"
        )

        self.double_layer_indexs = double_layer_indexs
        self.single_layer_indexs = single_layer_indexs

        self.double_transformer_distill_blocks = nn.ModuleList(
            [copy.deepcopy(self.transformer_blocks[start]) for start, _ in self.double_layer_indexs]
        )
        self.single_transformer_distill_blocks = nn.ModuleList(
            [copy.deepcopy(self.single_transformer_blocks[start]) for start, _ in self.single_layer_indexs]
        )

        self.double_transformer_distill_blocks.train()
        self.single_transformer_distill_blocks.train()
        self.double_transformer_distill_blocks.requires_grad_(True)
        self.single_transformer_distill_blocks.requires_grad_(True)
        ...
        self.enable_distill_inference(False)

    def get_distill_config(self) -> dict[str, list[list[int]]]:
        return {
            "double_layer_indexs": [list(layer_index) for layer_index in getattr(self, "double_layer_indexs", [])],
            "single_layer_indexs": [list(layer_index) for layer_index in getattr(self, "single_layer_indexs", [])],
        }

    def get_distill_lora_target_modules(self, target_modules: list[str]) -> list[str]:
        if not self._has_distill_blocks():
            raise RuntimeError("init_distill(...) must be called before collecting distillation LoRA target modules.")

        distill_prefixes = ("double_transformer_distill_blocks.", "single_transformer_distill_blocks.")
        matched_target_modules: list[str] = []

        for module_name, module in self.named_modules():
            if not isinstance(module, nn.Linear):
                continue
            if not module_name.startswith(distill_prefixes):
                continue
            if any(module_name.endswith(target_module) for target_module in target_modules):
                matched_target_modules.append(module_name)

        if len(matched_target_modules) == 0:
            raise ValueError(
                f"None of the requested LoRA target modules {target_modules} were found inside the distillation blocks."
            )

        return matched_target_modules

    @staticmethod
    def _build_distill_interval_maps(
        layer_indexs: list[tuple[int, int]],
    ) -> tuple[dict[int, dict[str, int]], dict[int, dict[str, int]]]:
        start_to_interval: dict[int, dict[str, int]] = {}
        end_to_interval: dict[int, dict[str, int]] = {}
        for student_idx, (start, end) in enumerate(layer_indexs):
            interval_state = {"student_idx": student_idx, "start": start, "end": end}
            start_to_interval[start] = interval_state
            end_to_interval[end] = interval_state

        return start_to_interval, end_to_interval

    def _normalized_feature_mse(self, student_hidden_states: torch.Tensor, teacher_hidden_states: torch.Tensor):
        student_hidden_states = _l2_normalize_last_dim(student_hidden_states.float())
        teacher_hidden_states = _l2_normalize_last_dim(teacher_hidden_states.float())
        return F.mse_loss(student_hidden_states, teacher_hidden_states, reduction="none").sum(dim=-1).mean()

    def _run_double_block(
        self,
    ...
    def _distill_forward_impl(
        ...
        if not self._has_distill_blocks():
            raise RuntimeError("init_distill(...) must be called before distill_forward(...).")

        num_txt_tokens = encoder_hidden_states.shape[1]
        double_start_to_interval, double_end_to_interval = self._build_distill_interval_maps(self.double_layer_indexs)
        single_start_to_interval, single_end_to_interval = self._build_distill_interval_maps(self.single_layer_indexs)
        ...
        loss_list: list[tuple[Any, ...]] = []
        active_double_states: dict[int, tuple[torch.Tensor, torch.Tensor]] = {}
        ...
        for index_block, block in enumerate(self.transformer_blocks):
            ...
            interval_state = double_start_to_interval.get(index_block)
            if interval_state is not None:
                distill_block = self.double_transformer_distill_blocks[interval_state["student_idx"]]
                active_double_states[index_block] = self._run_double_block(
                    distill_block,
                    input_hidden_states,
            ...
            interval_state = double_end_to_interval.get(index_block)
            if interval_state is not None:
                student_encoder_hidden_states, student_hidden_states = active_double_states.pop(interval_state["start"])
                text_loss = self._normalized_feature_mse(student_encoder_hidden_states, encoder_hidden_states)
                image_loss = self._normalized_feature_mse(student_hidden_states, hidden_states)
                loss = loss + text_loss + image_loss
                loss_list.append(
                    ("double", interval_state["start"], interval_state["end"], text_loss.item(), image_loss.item())
                )
        ...
        active_single_states: dict[int, torch.Tensor] = {}
        for index_block, block in enumerate(self.single_transformer_blocks):
            ...
            interval_state = single_start_to_interval.get(index_block)
            if interval_state is not None:
                distill_block = self.single_transformer_distill_blocks[interval_state["student_idx"]]
                active_single_states[index_block] = self._run_single_block(
                    distill_block,
                    input_hidden_states,
                    single_stream_mod,
                    concat_rotary_emb,
            ...
            interval_state = single_end_to_interval.get(index_block)
            if interval_state is not None:
                student_hidden_states = active_single_states.pop(interval_state["start"])
                single_loss = self._normalized_feature_mse(student_hidden_states, hidden_states)
                loss = loss + single_loss
                loss_list.append(("single", interval_state["start"], interval_state["end"], single_loss.item()))
        ...
        if kv_cache_mode == "extract":
            if not return_dict:
                return (output, loss, loss_list, kv_cache)
            return Flux2Transformer2DModelOutput(sample=output, kv_cache=kv_cache), loss, loss_list

        if not return_dict:
            return (output, loss, loss_list)

        return Flux2Transformer2DModelOutput(sample=output), loss, loss_list

    def _distill_infer_impl(
        self,
        hidden_states: torch.Tensor,
        encoder_hidden_states: torch.Tensor = None,
        timestep: torch.LongTensor = None,
        ...
        if not self._has_distill_blocks():
            raise RuntimeError("init_distill(...) must be called before distill_infer(...).")
        if kv_cache_mode is not None or kv_cache is not None or num_ref_tokens != 0 or ref_fixed_timestep != 0.0:
            raise NotImplementedError("distill_infer(...) currently supports only the standard Flux2 forward path.")
        ...
        distill_double_idx = 0
        for index_block, block in enumerate(self.transformer_blocks):
            use_block = True
            if distill_double_idx < len(self.double_layer_indexs):
                start, end = self.double_layer_indexs[distill_double_idx]
                if start <= index_block <= end:
                    if index_block == start:
                        block = self.double_transformer_distill_blocks[distill_double_idx]
                    else:
                        use_block = False
                    if index_block == end:
                        distill_double_idx += 1
        ...
        distill_single_idx = 0
        for index_block, block in enumerate(self.single_transformer_blocks):
            use_block = True
            if distill_single_idx < len(self.single_layer_indexs):
                start, end = self.single_layer_indexs[distill_single_idx]
                if start <= index_block <= end:
                    if index_block == start:
                        block = self.single_transformer_distill_blocks[distill_single_idx]
                    else:
                        use_block = False
                    if index_block == end:
                        distill_single_idx += 1
    ...
    def forward(
        ...
        """
        if getattr(self, "_use_distill_inference", False) and self._has_distill_blocks() and kv_cache_mode is None:
            return self._distill_infer_impl(
                hidden_states=hidden_states,
                encoder_hidden_states=encoder_hidden_states,
                timestep=timestep,
                img_ids=img_ids,
    ...
    def distill_forward(
        ...
        return self._distill_forward_impl(
            hidden_states=hidden_states,
            encoder_hidden_states=encoder_hidden_states,
            timestep=timestep,
            img_ids=img_ids,
