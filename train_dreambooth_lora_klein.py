...
def parse_json_list_arg(raw_value: str | None) -> list[list[int]] | None:
...
def normalize_distill_config(
    double_layer_indexs: list[list[int]] | None = None,
    single_layer_indexs: list[list[int]] | None = None,
) -> dict[str, list[list[int]]]:
    return {
        "double_layer_indexs": [list(layer_index) for layer_index in (double_layer_indexs or [])],
        "single_layer_indexs": [list(layer_index) for layer_index in (single_layer_indexs or [])],
    }


def has_distill_intervals(distill_config: dict[str, list[list[int]]] | None) -> bool:
    if distill_config is None:
        return False

    return len(distill_config.get("double_layer_indexs", [])) > 0 or len(distill_config.get("single_layer_indexs", [])) > 0


def save_distill_config(output_dir: str, distill_config: dict[str, list[list[int]]] | None):
    if output_dir is None or not has_distill_intervals(distill_config):
        return
...
def load_distill_config(output_dir: str) -> dict[str, list[list[int]]] | None:
    config_path = os.path.join(output_dir, "distill_config.json")
    ...
    return normalize_distill_config(
        double_layer_indexs=distill_config.get("double_layer_indexs"),
        single_layer_indexs=distill_config.get("single_layer_indexs"),
    )


def format_distill_loss_list(loss_list: list[tuple[Any, ...]]) -> str:
    formatted_lines: list[str] = []
    for loss_entry in loss_list:
        if len(loss_entry) == 5 and loss_entry[0] == "double":
            _, start, end, text_loss, image_loss = loss_entry
            formatted_lines.append(f"[double {start:02d}-{end:02d}] text={text_loss:.5f}, image={image_loss:.5f}")
        elif len(loss_entry) == 4 and loss_entry[0] == "single":
            _, start, end, single_loss = loss_entry
            formatted_lines.append(f"[single {start:02d}-{end:02d}] hidden={single_loss:.5f}")
        else:
            formatted_lines.append(str(loss_entry))
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--distill_double_layer_indexs",
        "--double_layer_indexs",
        type=str,
        default=None,
        help=(
            'Optional JSON list of double-stream distillation intervals, e.g. "[[0,2],[4,5]]". '
            "When set, LoRA training is restricted to the student double-stream blocks."
        ),
    )
    parser.add_argument(
        "--distill_single_layer_indexs",
        "--single_layer_indexs",
        type=str,
        default=None,
        help=(
            'Optional JSON list of single-stream distillation intervals, e.g. "[[0,1],[6,8]]". '
            "When set, LoRA training is restricted to the student single-stream blocks."
        ),
    )
...
def main(args):
    ...
    distill_config = normalize_distill_config(
        double_layer_indexs=parse_json_list_arg(args.distill_double_layer_indexs),
        single_layer_indexs=parse_json_list_arg(args.distill_single_layer_indexs),
    )
    if not has_distill_intervals(distill_config) and args.resume_from_checkpoint is not None:
        persisted_distill_config = load_distill_config(args.output_dir) if args.output_dir is not None else None
        if persisted_distill_config is not None:
            distill_config = persisted_distill_config

    is_distill_training = has_distill_intervals(distill_config)
    if is_distill_training:
        transformer.init_distill(
            double_layer_indexs=distill_config["double_layer_indexs"],
            single_layer_indexs=distill_config["single_layer_indexs"],
        )
        logger.info(
            "Enabled distillation LoRA training with %s double-stream interval(s) and %s single-stream interval(s).",
            len(distill_config["double_layer_indexs"]),
            len(distill_config["single_layer_indexs"]),
        )
        if accelerator.is_main_process:
            save_distill_config(args.output_dir, distill_config)
    ...
    if is_distill_training:
        target_modules = list(dict.fromkeys(transformer.get_distill_lora_target_modules(target_modules)))
    else:
    ...
    logger.info("LoRA will target %s transformer module(s).", len(target_modules))
    ...
    def save_model_hook(models, weights, output_dir):
        ...
        if accelerator.is_main_process:
            ...
            save_distill_config(output_dir, distill_config)

    def load_model_hook(models, input_dir):
        transformer_ = None
        checkpoint_distill_config = load_distill_config(input_dir)
        active_distill_config = checkpoint_distill_config if checkpoint_distill_config is not None else distill_config
        ...
        if not is_fsdp:
            ...
            if has_distill_intervals(active_distill_config):
                transformer_.init_distill(
                    double_layer_indexs=active_distill_config["double_layer_indexs"],
                    single_layer_indexs=active_distill_config["single_layer_indexs"],
                )
            transformer_.add_adapter(transformer_lora_config)

        if has_distill_intervals(active_distill_config) and hasattr(transformer_, "_has_distill_blocks"):
            if not transformer_._has_distill_blocks():
                transformer_.init_distill(
                    double_layer_indexs=active_distill_config["double_layer_indexs"],
                    single_layer_indexs=active_distill_config["single_layer_indexs"],
                )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(models_to_accumulate):
                ...
                loss_list = []
                if is_distill_training:
                    _, align_loss, loss_list = transformer.distill_forward_attn(
                ...
                if accelerator.sync_gradients:
                    params_to_clip = filter(lambda p: p.requires_grad, transformer.parameters())
            ...
            if align_loss is not None:
                logs["align_loss"] = align_loss.detach().item()
            ...
            if (
                ...
                logger.info("Distillation losses:\n  %s", format_distill_loss_list(loss_list))
        ...
        if accelerator.is_main_process:
            if args.validation_prompt is not None and epoch % args.validation_epochs == 0:
                validation_transformer = unwrap_model(transformer)
                if is_distill_training:
                    validation_transformer.enable_distill_inference(True)
                ...
                free_memory()
                if is_distill_training:
                    validation_transformer.enable_distill_inference(False)
    ...
    if accelerator.is_main_process:
        ...
        if should_run_final_inference:
            ...
            if is_distill_training:
                pipeline.transformer.init_distill(
                    double_layer_indexs=distill_config["double_layer_indexs"],
                    single_layer_indexs=distill_config["single_layer_indexs"],
                )
                pipeline.transformer.enable_distill_inference(True)
