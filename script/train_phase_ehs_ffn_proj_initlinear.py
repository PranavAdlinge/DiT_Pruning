...
from src.transformer_qwenimage_attn_ehs_ffn_proj  import QwenImageTransformer2DModel
...
def parse_args(input_args=None):
    ...
    parser.add_argument("--transformer_distill_path", type=str)
    parser.add_argument("--layer_indexs", type=parse_json_list)
    parser.add_argument("--ehs_same", type=parse_json_list)
...
def main(args):
    ...
    # train_device_id = local_rank
    # infer_device_id = torch.cuda.device_count() - 1 - train_device_id
    # accelerator.device = torch.device(f"cuda:{train_device_id}" )
    # accelerator.device = torch.device(f"cuda:{infer_device_id}" )
    # os.environ["ACCELERATE_TORCH_DEVICE"] = f"cuda:{train_device_id}" 
    ...
    transformer.init_distill(layer_indexs=args.layer_indexs, ehs_same=args.ehs_same)

    transformer_distill_path = args.transformer_distill_path
    state_dict = torch.load(transformer_distill_path, map_location="cpu")
    state_dict_new = {}
    for k,v in state_dict.items():
        k_new = k.replace("module.","")
        state_dict_new[k_new] = v
    print(f"load checkpoint {transformer_distill_path}")
    transformer.transformer_distill_blocks.load_state_dict(state_dict_new)
    del state_dict_new
    del state_dict
    ...
    params = list(transformer.ffn_proj_list.parameters()) + list(transformer.attn_proj_list.parameters()) + list(transformer.ehs_proj_list.parameters()) + list(transformer.block_proj_list.parameters())
    total = sum(p.numel() for p in params)
    print(f"****** train {total} params")
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(transformer):
                ...
                model_pred, align_loss, loss_list = transformer.init_linear(
                ...
                #     params_to_clip = list(transformer.attn_proj_list.parameters()) + list(transformer.ffn_proj_list.parameters())
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    if global_step % args.checkpointing_steps == 0:
                        ...
                        attn_proj_list = x.attn_proj_list.state_dict()
                        ffn_proj_list = x.ffn_proj_list.state_dict()
                        block_proj_list = x.block_proj_list.state_dict()
                        ehs_proj_list = x.ehs_proj_list.state_dict()
                        transformer_distill_blocks = x.transformer_distill_blocks.state_dict()
                        state_dict = {
                            "attn_proj_list": attn_proj_list,
                            "ffn_proj_list": ffn_proj_list,
                            "block_proj_list": block_proj_list,
                            "ehs_proj_list": ehs_proj_list,
                            "transformer_distill_blocks": transformer_distill_blocks,
                        }
                        os.makedirs(save_path, exist_ok=True)
            ...
            #     print("\nloss_list:\n  " + "\n  ".join(f"[layer_num] {i:2d} [index_block] {loss_[1]:2d} [index] {loss_[2]:2d}: [{loss_[3]}] {loss_[0]:8.5f}" for i, loss_ in enumerate(loss_list)))
